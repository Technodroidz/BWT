...
@section('title') Admin | UserCategory @endsection
    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="table-responsive userlist-table">
          <a href="{{route('admin.usercategory.create')}}">add</a>
          <table class="table card-table table-striped table-vcenter text-nowrap mb-0">
            ...
            <tbody>
              ...
              <tr>
                  ...
                  <td> 
                    <label for="{{ $row->id }}"> 
                    <input id="{{ $row->id }}" data-url="{{ route('admin.usercategory.changestatus')}}" name="onoffswitch" type="checkbox" data-id="{{ $row->id }}" <?= $row->active=='1' ? 'checked':'' ?> value="{{ $row->active }}">
                    </label>
                  </td>
                  <td>{{ changeDateFormat($row->created_at, 'M-d-Y') }}</td>
                  <td>                    
                      <a href="{{ route('admin.usercategory.edit',$row->id)}}">
                          <i class="far fa-pencil"></i>
                      </a>
                      <a href="{{ url('/admin/usercategory/delete/'.$row->id) }}" onclick="return confirm('Are you sure to delete this item?')">
...
<div class="sidebar sidebar-right sidebar-animate">
  <div class="panel panel-primary card mb-0 box-shadow">
    ...
    <div class="panel-body tabs-menu-body latest-tasks border-0">
        ...
        <form class="form-horizontal" action="{{url('admin/usercategory')}}">
              <div class="form-group">
                <input type="text" name="name" value="{{request('name')}}" class="form-control" placeholder="User Category Name">
              </div>
              <div class="form-group mb-0 mt-3 justify-content-end">
                <div>
                  
                  <button type="submit" class="btn btn-primary">Submit</button>
                  <a href={{url('admin/usercategory')}} class="btn btn-secondary float-right">Reset</a>
                </div>
              </div>
            </form>
    </div>
