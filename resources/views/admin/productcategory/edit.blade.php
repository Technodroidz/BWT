...
@section('title') Admin | Product Category Page | Edit  @endsection
    ...
    <div class="card">
      <div class="card-body">
        <div class="table-responsive userlist-table">
          ...
          <form class="form-horizontal"  method="post" action="{{ route('admin.product-category.update',$productcategorys->id) }}" enctype="multipart/form-data">
            @csrf
            <div class="row m-0">

            <div class="form-group col-sm-12">
              <label for="name">Name</label>
              <input type="text" class="form-control" id="name" value="{{old('name')??$productcategorys->name??''}}" name="name" placeholder="Enter Name here..">
            @error('name')
            ...
            <div class="form-group col-sm-12">
              ...
              <input type="text" class="form-control"  id="position" value="{{old('position')??$productcategorys->position??''}}" name="position" placeholder="Enter Position here...">
              @error('position')
              <span class="text-danger">{{$message}}</span>
            @enderror            
            </div>
            ...
            <div class="form-group col-sm-12">
              ...
              <input type="file" class="form-control"  id="thumbnail" accept=".jpg,.png,.jpeg,.gif" name="thumbnail">
              @error('thumbnail')
              <span class="text-danger">{{$message}}</span>
            @enderror 
            </div>
