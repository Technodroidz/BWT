        ...
        <div class="table-responsive userlist-table">
          ...
          <form class="form-horizontal"   method="post" action="{{ route('admin.category.update',$category->id) }}" enctype="multipart/form-data">
            ...
            <div class="form-group col-sm-12">
              ...
              <input type="text" class="form-control" id="name" value="{{ $category->name }}" name="name" onkeyup="convertToSlug(this.value)" placeholder="Enter Name">
            ...
            <div class="col-sm-12">
              <div class="input-field col s12"> 
                ...
                <select name="parent_id" id="parent_id" class="form-control" >
                  ...
                  <option value="1" labelVal="0" @if($category->parent_id ==1) {{ 'selected' }} @endif>Main Category</option>
                  @if(!empty($listData))
                  @foreach($listData as $key => $row)
                  <option style="font-weight: bolder;font-size: larger;" labelVal="{{ $row->position }}" value="{{ $row->id }}"  @if($category->parent_id == $row->id) {{ 'selected' }} @endif  >&nbsp;&nbsp;@if($row->parent_id !=1 && $key > 0) @for($i=0;$i<$row->position;$i++) {{ "-" }} @endfor  @endif{{ $row->name }}</option>
