...
@section('title') Admin | Teacher @endsection
...
<div class="breadcrumb-header justify-content-between">
  <div class="left-content">
    <div>
      <h4 class="main-content-title tx-22 mg-b-1 mg-b-lg-1">Teacher</h4>
          ...
          <table class="table card-table table-striped table-vcenter text-nowrap mb-0">
            <thead>
              <tr>
                ...
                <th class="wd-lg-20p"><span>Profile</span></th>
                <th class="wd-lg-20p"><span>Name</span></th>
                <th class="wd-lg-20p"><span>Email</span></th>
                <th class="wd-lg-20p"><span>Mobile</span></th>
                <th class="wd-lg-20p"><span>Language</span></th>
              ...
              <tr>
                <td>{{ $key+1 }}</td> 
                  ...
                  <td>
                     ...
                     <img alt="avatar" class="rounded-circle avatar-md mr-2" src="{{ $row->thumbnail}}"> 
                  </td>
                  <td>{{ $row->name ?? '-' }}</td>
                  <td>{{ $row->email ?? '-' }}</td>
                  <td>{{ $row->mobile ?? '-' }}</td>
                  ...
                  <td>
                    <label for="{{ $row->id }}"> 
                    <input id="{{ $row->id }}" data-url="{{ route('admin.teacher.changestatus')}}" name="onoffswitch" type="checkbox" data-id="{{ $row->id }}" <?= $row->status=='1' ? 'checked':'' ?> value="{{ $row->status }}">
                  ...
                  <td>                    
                      ...
                      <a href="{{  url('/admin/teacher/delete/'.$row->id) }}" onclick="return confirm('Are you sure to delete this item?')">
